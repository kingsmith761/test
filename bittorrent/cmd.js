...
function runDownload(torrentId) {
    ...
    client = new WebTorrent({ blocklist: argv.blocklist, iota: argv.iota })
    client.on('error', fatalError)

    const { out: path, announce, iota } = argv
    const torrent = client.add(torrentId, { path, announce, iota })
...
function runSeed(input) {
    ...
    const client = new WebTorrent({ blocklist: argv.blocklist, iota: argv.iota })
    client.on('error', fatalError)

    client.seed(input, { announce: argv.announce, iota: argv.iota }, torrent => {
        if (argv.quiet) {
